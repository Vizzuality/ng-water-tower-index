...
function drawThreeGeo(json, radius, shape, materalOptions, container) {
  ...
  var coordinate_array = []; //Re-usable array to hold coordinate values. This is necessary so that you can add
...
var GlobeLayerKapsule = Kapsule({
  ...
  stateInit: function stateInit() {
    ...
    {
      // add graticules
      graticulesObj = new THREE$1.Object3D();
      drawThreeGeo({
...
var gradientShaders = {
  uniforms: {
    ...
    dashTranslate: {
      value: 0 // used for animating the dash
  ...
  update: function update(state) {
    ...
    threeDigest(singlePolygons, state.scene, {
      ...
      createObj: function createObj() {
        ...
        var mesh = new THREE$5.Mesh(undefined, [
        ...
        var line = new Line(undefined, line_material);
      ...
      updateObj: function updateObj(obj, _ref) {
        ...
        [sideColor, capColor].forEach(function (color, materialIdx) {
          ...
          var material = mesh.material[materialIdx];
          ...
          obj.children.map(c => {
            if (c.type === 'Mesh') {
              ...
              for (var point_num = 0; point_num < coordinatesArray.length; point_num++) {
                convertToSphereCoords(coordinatesArray[point_num], GLOBE_RADIUS * (1 + alt));
              }

              for (var i = 0; i < x_values.length; i++) {
                c.geometry.vertices.push(new THREE.Vector3(x_values[i], y_values[i], z_values[i]));
              }
            }
          })
        };
  ...
  update: function update(state) {
    ...
    threeDigest(state.labelsData, state.scene, {
      ...
      createObj: function createObj() {
        ...
        var obj = new THREE$6.Group(); // container
...
var Globe = Kapsule({
  props: _objectSpread({}, linkedGlobeLayerProps, linkedPointsLayerProps, linkedArcsLayerProps, linkedHexBinLayerProps, linkedPolygonsLayerProps, linkedLabelsLayerProps, linkedCustomLayerProps),
  methods: {
  ...
  init: function init(threeObj, state, _ref8) {
    ...
    threeObj.add(state.scene = new THREE$7.Group()); // add globe layer group

    var globeG = new THREE$7.Group();
    state.scene.add(globeG);
    state.globeLayer(globeG); // add points layer group

    var pointsG = new THREE$7.Group();
    state.scene.add(pointsG);
    state.pointsLayer(pointsG); // add arcs layer group

    var arcsG = new THREE$7.Group();
    state.scene.add(arcsG);
    state.arcsLayer(arcsG); // add hexbin layer group

    var hexBinG = new THREE$7.Group();
    state.scene.add(hexBinG);
    state.hexBinLayer(hexBinG); // add polygons layer group

    var polygonsG = new THREE$7.Group();
    state.scene.add(polygonsG);
    state.polygonsLayer(polygonsG); // add labels layer group

    var labelsG = new THREE$7.Group();
    state.scene.add(labelsG);
    state.labelsLayer(labelsG); // add custom layer group

    var customG = new THREE$7.Group();
    state.scene.add(customG);
    state.customLayer(customG); // animate build in, one time only
    ...
    if (state.animateIn) {
      setTimeout(function () {
        ...
        new TWEEN.Tween({
          ...
          k: 1
        }, 600).easing(TWEEN.Easing.Quadratic.Out).onUpdate(function (_ref9) {
          var k = _ref9.k;
          return state.scene.scale.set(k, k, k);
        }).start();
        var rotAxis = new THREE$7.Vector3(0, 1, 0);
        new TWEEN.Tween({
          rot: Math.PI * 2
        }).to({
          rot: 0
        }, 1200).easing(TWEEN.Easing.Quintic.Out).onUpdate(function (_ref10) {
          var rot = _ref10.rot;
