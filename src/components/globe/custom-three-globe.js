...
function createLineGeometry(geometry, coords, radius, altitude) {
  var x_values = [];
  var y_values = [];
  var z_values = [];
  ...
  for (var i = 0; i < x_values.length; i++) {
    geometry.vertices.push(new THREE$5.Vector3(x_values[i], y_values[i], z_values[i]));
  }
}
  ...
  update: function update(state) {
    ...
    threeDigest(singlePolygons, state.scene, {
      ...
      createObj: function createObj() {
        ...
        var mesh = new THREE$6.Mesh(undefined, [new THREE$6.MeshLambertMaterial({
        ...
        obj.add(mesh);
        var line_material = new THREE$6.LineBasicMaterial({
          color: 'black'
        });
        var line = new THREE$6.Line(undefined, line_material);
        obj.add(line);
        ...
        var applyUpdate = function applyUpdate(td) {
          ...
          obj.children.map(function (c) {
            if (c.type === 'Mesh') {
              ...
              c.geometry = new THREE$6.Geometry();
  ...
  update: function update(state) {
    ...
    threeDigest(state.labelsData, state.scene, {
      ...
      createObj: function createObj() {
        ...
        var obj = new THREE$7.Group(); // container
  ...
  update: function update(state) {
    if (state.animateIn) {
      setTimeout(function () {
        ...
        var rotAxis = new THREE$8.Vector3(0, 1, 0);
        new TWEEN.Tween({
          rot: Math.PI * 2
        }).to({
          rot: 0
